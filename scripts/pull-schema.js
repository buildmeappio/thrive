...
function promptForToken() {
  return new Promise(resolve => {
    ...
    rl.question('GitHub token not found. Please enter your GitHub token: ', token => {
      rl.close();
      resolve(token.trim());
    });
  });
...
const MAX_CONCURRENT_DOWNLOADS = parseInt(process.env.MAX_CONCURRENT_DOWNLOADS || '50', 10);
...
async function collectFiles(srcDir, destDir, token, fileList = []) {
  try {
    ...
    console.error(`Failed to fetch directory contents: ${srcDir}`, error.message);
    return fileList;
  }
}
    ...
    for (const result of batchResults) {
      ...
      if (result.status === 'fulfilled') {
        ...
        if (success) {
          ...
          console.error(`✗ [${completed}/${total}] Failed: ${filePath} - ${error}`);
...
async function main() {
  ...
  console.log(`Fetching prisma folder from ${OWNER}/${REPO} (branch: ${BRANCH})...`);
