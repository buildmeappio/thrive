...
class DepartmentSeeder {
  ...
  private async createDepartments(data: DepartmentData[]): Promise<void> {
    ...
    for (const deptData of data) {
      ...
      console.log(
        `✅ Created new organization department: "${organizationDepartment.name}" (ID: ${organizationDepartment.id})`
      );
    }
  }
  ...
  public async cleanupOldOrganizationDepartments() {
    ...
    console.log(
      `⚠️ Found ${oldDepartments.length} old organization departments that might need cleanup:`
    );
