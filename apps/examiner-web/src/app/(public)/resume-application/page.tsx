  ...
  useEffect(() => {
    ...
    const verifyAndLoad = async () => {
      try {
        ...
        if (!result.success) {
          const errorMessage =
            'message' in result ? result.message : 'Failed to verify resume token';
        ...
        const examinerData = {
          ...
          isConsentToBackgroundVerification: application.consentBackgroundVerification || false,
        ...
        toast.success('Application loaded successfully!', {
          description: 'Your application has been restored. You can continue where you left off.',
          duration: 5000,
        });
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#F4FBFF] px-4">
        <div className="w-full max-w-md rounded-[20px] bg-white p-8 text-center shadow-lg md:rounded-[40px] md:p-12">
          ...
          <h1 className="mb-4 text-2xl font-semibold text-[#140047] md:text-3xl">{error.title}</h1>
          <p className="mb-8 text-base text-gray-600 md:text-lg">{error.message}</p>
          <Button
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#F4FBFF]">
        <div className="text-center">
          ...
          <p className="mt-4 text-lg text-gray-600">Loading your application...</p>
        </div>
      </div>
    );
  }
