...
export default function DateRangeFilter({
  ...
  useEffect(() => {
    if (isOpen && popupRef.current && dropdownRef.current) {
      ...
      setPositionAbove(spaceBelow < popupHeight + 20 && spaceAbove > spaceBelow);
    }
  }, [isOpen, isMobile]);
    ...
    <div ref={dropdownRef} className={cn('relative', className)}>
      ...
      {isOpen && (
        <div
          ref={popupRef}
          className={cn(
            'absolute left-1/2 z-50 w-[90vw] max-w-[600px] -translate-x-1/2 rounded-lg border border-gray-200 bg-white p-3 shadow-lg sm:w-[600px] sm:p-6',
            isMobile ? 'mb-2 max-h-[85vh] overflow-y-auto' : 'max-h-none',
