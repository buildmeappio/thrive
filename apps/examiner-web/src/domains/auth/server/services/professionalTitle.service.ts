...
class ProfessionalTitleService {
  async getProfessionalTitles() {
    try {
      ...
      if (isDatabaseError) {
        ...
        console.error('Database error in getProfessionalTitles:', error);
      ...
      throw HttpError.fromError(error, ErrorMessages.PROFESSIONAL_TITLES_NOT_FOUND, 500);
    }
  }
}

export default new ProfessionalTitleService();
