    ...
    setItem: (name: string, value: unknown): void => {
      ...
      try {
        ...
        if (state?.state?.data) {
          ...
          if (sanitizedData.cvResume && sanitizedData.cvResume instanceof File) {
            sanitizedData.cvResume = null;
          }
          if (sanitizedData.signedNDA && sanitizedData.signedNDA instanceof File) {
            sanitizedData.signedNDA = null;
          }
          if (sanitizedData.insuranceProof && sanitizedData.insuranceProof instanceof File) {
...
export const useRegistrationStore = create<Store>()(
  persist(
    set => ({
      ...
      yearsOfExperience: [],
      setYearsOfExperience: (years: YearsOfExperience[]) => set({ yearsOfExperience: years }),
      ...
      loadExaminerData: (examinerData: ExaminerData) => {
        ...
        const mappedData: Partial<RegistrationData> = {
          ...
          province: examinerData.address?.province || examinerData.provinceOfResidence || '',
          ...
          // Step 2: Address
          address: examinerData.address?.address || examinerData.mailingAddress || '',
          ...
          licenseIssuingProvince:
            examinerData.provinceOfLicensure || examinerData.licenseIssuingProvince || '',
          ...
          // Step 6: Legal
          consentBackgroundVerification: examinerData.isConsentToBackgroundVerification || false,
