...
export const specialtyPreferencesSchema = z.object({
  specialty: z.array(z.string()).min(1, { message: 'Please select at least one specialty' }),
  assessmentTypes: z
    .array(z.string())
    .min(1, { message: 'Please select at least one assessment type' }),
  preferredFormat: z.string().min(1, { message: 'Please select a preferred format' }),
  regionsServed: z.array(z.string()).min(1, { message: 'Please select at least one region' }),
  languagesSpoken: z.array(z.string()).min(1, { message: 'Please select at least one language' }),
});

export type SpecialtyPreferencesInput = z.infer<typeof specialtyPreferencesSchema>;
...
export const bookingOptionsSchema = z.object({
  maxIMEsPerWeek: z.string().min(1, { message: 'Maximum IMEs per week is required' }),
  minimumNotice: z.string().min(1, { message: 'Minimum notice required is required' }),
});
