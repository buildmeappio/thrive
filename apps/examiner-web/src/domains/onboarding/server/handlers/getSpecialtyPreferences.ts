...
const getSpecialtyPreferences = async (payload: GetSpecialtyPreferencesInput) => {
  ...
  const languageIds = examinerProfile.examinerLanguages.map(el => el.languageId);
  ...
  return {
    ...
    data: {
      ...
      regionsServed: profile.preferredRegions ? profile.preferredRegions.split(',') : [],
