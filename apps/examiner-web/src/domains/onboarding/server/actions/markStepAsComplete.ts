...
export const markStepAsCompleteAction = async (data: {
  ...
  try {
    ...
    const updatedActivationStep = addCompletedStep(profile.activationStep, data.stepId);
    ...
    return {
      ...
      message:
        (error instanceof Error ? error.message : undefined) || 'Failed to mark step as complete',
    };
  }
};
