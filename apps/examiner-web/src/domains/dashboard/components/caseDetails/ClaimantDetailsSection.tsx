...
export default function ClaimantDetailsSection({ claimant }: ClaimantDetailsSectionProps) {
  return (
    <AccordionItem value="claimant" className="border-b border-[#EDEDED]">
      ...
      <AccordionContent className="pb-0 pt-0">
        <div className="space-y-2">
          {claimant.claimType && <DetailRow label="Type of Claim" value={claimant.claimType} />}
          <DetailRow label="First Name" value={claimant.firstName} />
          <DetailRow label="Last Name" value={claimant.lastName} />
          {claimant.dateOfBirth && (
            <DetailRow label="Date of Birth" value={formatDateShort(claimant.dateOfBirth)} />
          )}
          {claimant.gender && <DetailRow label="Gender" value={capitalizeWords(claimant.gender)} />}
          {claimant.phoneNumber && <DetailRow label="Phone" value={claimant.phoneNumber} />}
          {claimant.emailAddress && (
            <DetailRow label="Email Address" value={claimant.emailAddress} />
          )}
          {claimant.address && (
            <DetailRow label="Address Lookup" value={formatAddress(claimant.address)} />
          )}
          {claimant.relatedCasesDetails && (
            <DetailRow label="Related Cases" value={claimant.relatedCasesDetails} />
          )}
          {claimant.familyDoctorName && (
            <DetailRow label="Family Doctor" value={claimant.familyDoctorName} />
          )}
          {claimant.familyDoctorEmailAddress && (
            <DetailRow label="Email Address" value={claimant.familyDoctorEmailAddress} />
          )}
          {claimant.familyDoctorPhoneNumber && (
            <DetailRow label="Phone" value={claimant.familyDoctorPhoneNumber} />
          )}
          {claimant.familyDoctorPhoneNumber && (
            <DetailRow label="Fax No." value={claimant.familyDoctorPhoneNumber} />
          )}
        </div>
      </AccordionContent>
    </AccordionItem>
