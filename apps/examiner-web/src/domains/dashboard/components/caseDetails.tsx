...
export default function CaseDetails({ data, examinerProfileId }: CaseDetailsProps) {
      ...
      <div className="mx-auto max-w-[1800px]">
        ...
        <div className="mb-6 flex items-center justify-between">
          ...
          {data.reportStatus === 'PENDING' && (
            <div className="flex items-center gap-2 rounded-full border border-[#00A8FF] bg-[#E6F6FF] px-4 py-2">
              <svg
              ...
              <span className="text-sm font-semibold text-[#00A8FF]">Report Pending</span>
            </div>
          )}
          ...
          {data.reportStatus === 'DRAFT' && (
            <div className="flex items-center gap-2 rounded-full border border-[#FFA500] bg-[#FFF4E6] px-4 py-2">
              <svg
              ...
              <span className="text-sm font-semibold text-[#FFA500]">Report Draft</span>
            </div>
          )}
          {data.reportStatus === 'SUBMITTED' && (
            <div className="flex items-center gap-2 rounded-full border border-[#10B981] bg-[#ECFDF5] px-4 py-2">
              <Check className="h-4 w-4 text-[#10B981]" strokeWidth={3} />
              <span className="text-sm font-semibold text-[#10B981]">Report Submitted</span>
            </div>
          )}
          {data.reportStatus === 'REVIEWED' && (
            <div className="flex items-center gap-2 rounded-full border border-[#00A8FF] bg-[#E6F6FF] px-4 py-2">
              <Check className="h-4 w-4 text-[#00A8FF]" strokeWidth={3} />
              <span className="text-sm font-semibold text-[#00A8FF]">Reviewed</span>
            </div>
          )}
          {data.reportStatus === 'APPROVED' && (
            <div className="flex items-center gap-2 rounded-full border border-[#10B981] bg-[#ECFDF5] px-4 py-2">
              <Check className="h-4 w-4 text-[#10B981]" strokeWidth={3} />
              <span className="text-sm font-semibold text-[#10B981]">Report Approved</span>
            </div>
          )}
          {data.reportStatus === 'REJECTED' && (
            <div className="flex items-center gap-2 rounded-full border border-[#DC2626] bg-[#FEF2F2] px-4 py-2">
              <svg
              ...
              <span className="text-sm font-semibold text-[#DC2626]">Report Rejected</span>
            </div>
          )}
          {data.status === 'PENDING' && (
            <div className="flex items-center gap-2 rounded-full border border-[#FFA500] bg-[#FFF4E6] px-4 py-2">
              ...
              <span className="text-sm font-semibold text-[#FFA500]">Pending Review</span>
            </div>
          )}
        </div>
        ...
        <div className="mb-6 rounded-[29px] bg-white p-6 shadow-[0_0_36.92px_rgba(0,0,0,0.08)]">
          <Accordion type="single" collapsible className="w-full" defaultValue="claimant">
            ...
            {data.insurance && <InsuranceDetailsSection insurance={data.insurance} />}

            {data.legalRepresentative && (
              <LegalRepresentativeSection legalRepresentative={data.legalRepresentative} />
            )}
        ...
        {data.reportStatus === 'DRAFT' ? (
          <div className="mt-6 flex flex-col justify-end gap-4 sm:flex-row">
            <Button
              ...
              className="h-[48px] cursor-pointer rounded-[20px] bg-gradient-to-r from-[#01F4C8] to-[#00A8FF] px-6 font-medium text-white transition-opacity hover:opacity-90"
            >
              Continue Draft
            </Button>
          </div>
        ...
        ) : (data.status === 'ACCEPT' && !data.reportStatus) || data.reportStatus === 'PENDING' ? (
          <div className="mt-6 flex flex-col justify-end gap-4 sm:flex-row">
            <Button
              onClick={() => {
                router.push(`/appointments/${data.bookingId}/prepare-report`);
          ...
          <div className="mt-6 flex flex-col justify-end gap-4 sm:flex-row">
            <Button
              ...
              disabled={isLoading}
              variant="outline"
              className="h-[48px] cursor-pointer rounded-[20px] border-2 border-[#00A8FF] bg-white px-6 font-medium text-[#00A8FF] transition-colors hover:bg-[#F0F8FF]"
            >
              Accept Case
            </Button>
            <Button
              ...
              className="h-[48px] cursor-pointer rounded-[20px] bg-[#DC2626] px-6 font-medium text-white transition-colors hover:bg-[#B91C1C]"
            >
              Decline Offer
            </Button>
          </div>
