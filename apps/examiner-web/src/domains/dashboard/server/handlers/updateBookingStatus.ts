...
const updateBookingStatus = async (
  ...
  try {
    ...
    if (!bookingId || !examinerProfileId || !status) {
      throw HttpError.badRequest('Booking ID, Examiner Profile ID, and Status are required');
    }

    await caseDetailsService.updateBookingStatus(bookingId, examinerProfileId, status, message);

    return {
      success: true,
      message: `Case ${
        status === 'ACCEPT' ? 'accepted' : status === 'DECLINE' ? 'declined' : 'requested more info'
      } successfully`,
    };
  } catch (error: unknown) {
