...
export function isStepOpen(stepContainer: HTMLElement): boolean {
  const stepButton = stepContainer.querySelector('button') as HTMLButtonElement | null;
...
export function handleStepButtonClick(
  ...
  if (!isStepAlreadyOpen) {
    ...
    setTimeout(() => {
      ...
      console.log('[Tour] Button state after click - classes:', stepButton.className);
          ...
          for (let i = buttonIndex + 1; i < allChildren.length; i++) {
            ...
            if (sibling.tagName === 'DIV' && sibling.children.length > 0) {
              ...
              const hasActiveStyle =
                ...
                getComputedStyle(stepButton).borderColor.includes('rgb(0, 168, 255)');
      ...
      const checkStepOpen = () => {
        ...
        if (buttonParent) {
          ...
          if (hasActiveStyle && !stepIsOpen) {
            ...
            console.log('[Tour] Button has active style but form not found yet');
          }
        }
        ...
        if (stepIsOpen) {
          ...
          observer.disconnect();
          console.warn('[Tour] Step did not open after max attempts, continuing anyway');
          setIsWaitingForStep(false);
          document.body.style.overflow = '';
          pendingStepRef.current = null;
        } else {
