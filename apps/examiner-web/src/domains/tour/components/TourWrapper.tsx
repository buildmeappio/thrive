...
export function TourWrapper({
  ...
  const handleCallback = useCallback(
    ...
    [tourType, steps, handleTourComplete, handleTourSkip, stopTour, handleStepChange]
  );
  ...
  const getTarget = useCallback(
    (target: string | HTMLElement | (() => HTMLElement)): HTMLElement => {
        ...
        if (!element) {
          ...
          if (buttonElement && buttonElement instanceof HTMLElement) {
            ...
            if (!wrapper && buttonElement.parentElement) {
              buttonElement.parentElement.setAttribute('data-tour', 'complete-onboarding-button');
      ...
      const defaultElement = document.querySelector(targetString) as HTMLElement | null;
      if (!defaultElement || !(defaultElement instanceof HTMLElement)) {
        ...
        const tourAttr = targetString.match(/data-tour="([^"]+)"/)?.[1] || 'unknown';
