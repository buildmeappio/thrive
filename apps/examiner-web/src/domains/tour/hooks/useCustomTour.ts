...
export function useCustomTour({
  ...
  const findAndPositionElement = useCallback(
    (stepIndex: number) => {
      ...
      if (!element) {
        console.warn(`[Tour] Element not found for step ${stepIndex}:`, targetSelector);
        return null;
      }
      ...
      if (rect.width === 0 || rect.height === 0) {
        console.warn(`[Tour] Element has zero dimensions for step ${stepIndex}`);
        return null;
      }
      ...
      if (!targetSelector.includes('settings-button')) {
        ...
        if (!isInViewport) {
          ...
          setTimeout(() => {
            ...
            const newPosition = calculateTooltipPosition(element, step.placement || 'bottom');
  ...
  const nextStep = useCallback(async () => {
    if (currentStepIndex < steps.length - 1) {
      ...
      const isNextSettingsStep =
        typeof nextStep?.target === 'string' && nextStep.target.includes('settings-button');
      ...
      if (isNextSettingsStep && !currentPath.includes(settingsPath)) {
        console.log('[Tour] Navigating to settings page before showing step', nextIndex);
  ...
  const prevStep = useCallback(() => {
    if (currentStepIndex > 0) {
      ...
      const isCurrentSettingsStep =
        typeof currentStep?.target === 'string' && currentStep.target.includes('settings-button');
      const isPrevSettingsStep =
        typeof prevStep?.target === 'string' && prevStep.target.includes('settings-button');
      const currentPath = window.location.pathname;
      ...
      if (isCurrentSettingsStep && !isPrevSettingsStep && currentPath.includes(settingsPath)) {
        console.log('[Tour] Navigating back to dashboard page before showing step', prevIndex);
