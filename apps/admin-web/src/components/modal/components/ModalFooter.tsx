...
export default function ModalFooter({
  ...
  return (
    <div className="flex flex-shrink-0 justify-end gap-3 border-t border-gray-200 p-5 pt-4 sm:px-[45px] sm:pb-[40px]">
      {step === 1 && (
        <Step1Footer
          isLoading={isLoading}
...
function Step1Footer({
  ...
  return (
    <>
      <button type="button" onClick={onClose} disabled={isLoading} className={BUTTON_SECONDARY}>
        Cancel
      </button>
      <button
        type="button"
...
function Step2Footer({ isLoading, feeStructureData, onBack, onSubmit }: Step2FooterProps) {
  return (
    <>
      <button type="button" onClick={onBack} disabled={isLoading} className={BUTTON_SECONDARY}>
        Back
      </button>
      <button
        type="button"
...
function Step3Footer({ isLoading, onBack, onSubmit }: Step3FooterProps) {
  return (
    <>
      ...
      <button type="button" onClick={onSubmit} disabled={isLoading} className={BUTTON_PRIMARY}>
        {isLoading ? (
          <>
            <Loader2 className="h-4 w-4 animate-spin" />
...
function Step4Footer({ isLoading, contractId, onBack, onSend }: Step4FooterProps) {
  return (
    <>
      <button type="button" onClick={onBack} disabled={isLoading} className={BUTTON_SECONDARY}>
        Back
