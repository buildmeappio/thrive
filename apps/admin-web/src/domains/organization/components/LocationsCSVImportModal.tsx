...
interface LocationsCSVImportModalProps {
  open: boolean;
  onClose: () => void;
...
const LocationsCSVImportModal: React.FC<LocationsCSVImportModalProps> = ({
  open,
  onClose,
  organizationId,
  onSubmit,
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const result = await locationActions.importLocationsFromCSV({
        organizationId,
        csvText: text,
      });
      ...
      if (result.success) {
        toast.success(
          `Successfully imported ${result.created} new location(s) and updated ${result.updated} existing location(s)`
        );
        setTimeout(() => {
          onClose();
          onSubmit();
        ...
        if (result.successful > 0) {
          toast.warning(
            `Imported ${result.created} new location(s) and updated ${result.updated} existing location(s), but ${result.failed} failed. Check the details below.`
          );
        } else {
          toast.error(`Failed to import locations. Check the errors below.`);
        }
      }
    } catch (error) {
  ...
  const handleDownloadTemplate = () => {
    ...
    downloadCSV(csvContent, 'locations-import-template.csv');
  };
            ...
            <div className="space-y-4">
              <div className="rounded-lg border-2 border-dashed border-gray-300 p-6 text-center">
                {file ? (
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="mt-2 text-xs text-gray-500">CSV file with location data</p>
                  </div>
                )}
              </div>
              ...
              <div className="rounded-lg border border-blue-200 bg-blue-50 p-4">
                ...
                <div className="mt-4 border-t border-blue-300 pt-4">
                  <p className="text-xs text-gray-600">
                    <strong>Required:</strong> name, timezone, addressLine1, city, state, postalCode
                    <br />
                    <strong>Optional:</strong> addressLine2, isActive (defaults to true)
                    <br />
                    <strong>Note:</strong> Locations with the same name and timezone will be updated
...
export default LocationsCSVImportModal;
