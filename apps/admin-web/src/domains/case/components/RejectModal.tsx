...
export default function RejectModal({
  ...
  const canSend =
    (messageToClaimant.trim().length > 0 || messageToOrganization.trim().length > 0) &&
    ...
    >
      ...
      >
        ...
        <button
          ...
          onClick={onClose}
          disabled={isLoading}
          className="absolute right-4 top-4 grid h-8 w-8 place-items-center rounded-full bg-[#D32F2F] focus:outline-none focus:ring-2 focus:ring-[#D32F2F]/40 disabled:opacity-50 sm:right-5 sm:top-5 sm:h-[32px] sm:w-[32px]"
        >
        ...
        <h2
          id={titleId}
          className="font-degular pr-10 text-xl font-[600] leading-[1.2] tracking-[-0.02em] text-[#D32F2F] sm:text-[28px]"
        >
          {title}
        </h2>
        ...
        <div className="mt-6 flex justify-end">
          <button
            ...
            className="font-poppins h-10 rounded-full bg-[#D32F2F] px-8 text-[14px] font-[500] tracking-[-0.02em] text-white transition-opacity hover:bg-[#C62828] hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50 sm:h-[46px] sm:px-10 sm:text-[16px]"
          >
