...
const approveExaminer = async (id: string) => {
  ...
  if (entityType === 'application') {
    ...
    const application = await applicationService.approveApplication(id, user.accountId);
    ...
    try {
      ...
      logger.error('⚠️ Failed to send approval email (but approval succeeded):', emailError);
    }
    ...
    if (!existingContract) {
      ...
      logger.log('✅ Contract already exists for examiner:', existingContract.id);
    }
