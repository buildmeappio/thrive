...
const confirmInterviewSlot = async (
  ...
  if (!user) {
    throw HttpError.unauthorized('You must be logged in to confirm interview slot');
  }
  ...
  try {
    ...
    const confirmedSlot = await interviewSlotService.confirmRequestedInterviewSlot(
      slotId,
      applicationId
    );
    ...
    try {
      if (application.email && application.firstName && application.lastName) {
        ...
        const interviewDate = new Date(confirmedSlot.startTime).toLocaleDateString('en-US', {
          weekday: 'long',
          year: 'numeric',
          month: 'long',
          day: 'numeric',
        ...
        const interviewTime = `${new Date(confirmedSlot.startTime).toLocaleTimeString('en-US', {
          hour: 'numeric',
          minute: '2-digit',
          hour12: true,
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : 'Failed to confirm interview slot',
    };
  }
};
