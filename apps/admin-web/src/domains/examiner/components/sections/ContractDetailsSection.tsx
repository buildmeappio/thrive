...
export const ContractDetailsSection = ({ contractData }: ContractDetailsSectionProps) => {
  if (
    !contractData ||
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        {Object.entries(contractData.fieldValues.custom).map(([key, value]) => {
          ...
          if (Array.isArray(value)) {
            ...
            if (customVar?.options) {
              const selectedLabels = value
                .map(val => {
                  const option = customVar.options?.find(opt => opt.value === val);
          ...
          return (
            <div
              key={key}
              className="flex w-full flex-col gap-1.5 rounded-lg bg-[#F6F6F6] px-3 py-2 sm:px-4"
            >
              <span className="break-words font-[Poppins] text-[14px] font-[400] leading-tight tracking-[-0.03em] text-[#4E4E4E] sm:text-[16px]">
                {label.includes('*') ? (
                  <>
                    {label.replace('*', '')}
              ...
              <span className="break-words font-[Poppins] text-[14px] font-[400] leading-tight tracking-[-0.03em] text-[#000080] sm:text-[16px]">
                {displayValue ?? '-'}
              </span>
            </div>
          );
