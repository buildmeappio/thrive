...
export const feeStructureSchema = z.object({
  name: z
    ...
    .refine(val => hasAtLeastOneLetter(val), 'Name must contain at least one letter'),
  description: z
    .string()
...
export const feeVariableBaseSchema = z.object({
  ...
  label: z
    ...
    .refine(val => hasAtLeastOneLetter(val), 'Label must contain at least one letter'),
  key: z
    .string()
    ...
    .trim(),
  type: z.enum(
    [FeeVariableType.MONEY, FeeVariableType.NUMBER, FeeVariableType.TEXT, FeeVariableType.BOOLEAN],
    {
  ...
  if (data.composite) {
    ...
    for (let i = 0; i < data.subFields.length; i++) {
      ...
      if (subField.defaultValue !== undefined) {
        ...
        } else if (subField.type === 'TEXT' && typeof subField.defaultValue !== 'string') {
          ctx.addIssue({
            code: z.ZodIssueCode.custom,
