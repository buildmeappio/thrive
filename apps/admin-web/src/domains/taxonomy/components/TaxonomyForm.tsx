...
const TaxonomyForm: React.FC<TaxonomyFormProps> = ({
  ...
  const renderField = (field: TaxonomyField) => {
    ...
    const isTimeField =
      isConfiguration && isValueField && configurationName === 'start_working_hour_time';
    ...
    switch (field.type) {
      ...
      case 'select':
        const options =
          field.name === 'examinationTypeId' ? examinationTypeOptions : field.options || [];
        return (
          <div key={field.name} className="space-y-2">
            <Label htmlFor={field.name}>
              {field.label} {field.required && <span className="text-red-500">*</span>}
            ...
            <Select
              ...
              disabled={isSubmitting}
            >
              <SelectTrigger className="h-10 rounded-full border-[#E5E7EB] focus:border-transparent focus:ring-2 focus:ring-[#00A8FF]">
                <SelectValue placeholder={field.placeholder} />
              </SelectTrigger>
              <SelectContent className="select-scrollbar max-h-[300px]">
                {options.map(option => (
            ...
            <Input
              ...
              {...register(field.name, {
                ...
                validate: value => {
                  ...
                  if (trimmedValue && !isValueField && !hasAlphanumeric(trimmedValue)) {
                    return `${field.label} must contain at least one letter or number`;
                  }

                  // Check maxLength if specified (skip for numeric value fields)
                  if (field.maxLength && !isValueField && trimmedValue.length > field.maxLength) {
                    return `${field.label} must not exceed ${field.maxLength} characters`;
                  }
              ...
              className={isConfigurationEdit ? 'cursor-not-allowed bg-gray-100' : ''}
              maxLength={field.maxLength && !isValueField ? field.maxLength : undefined}
            />
    ...
    <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-6">
      ...
      <div className="flex justify-end gap-4 pt-4">
        <Button
          type="submit"
          disabled={isSubmitting}
          className="min-w-[120px] rounded-full bg-gradient-to-r from-[#00A8FF] to-[#01F4C8]"
        >
