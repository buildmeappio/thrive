...
const formatTimeRange = (startTime: string | Date, endTime: string | Date): string => {
...
const ActionButton = ({ id }: { id: string }) => {
  return (
    <Link href={`/examiner/application/${id}`} className="w-full h-full cursor-pointer">
...
const SortableHeader = ({ column, children }: { column: Column<InterviewData, unknown>; children: React.ReactNode }) => {
...
const columnsDef = [
  ...
  },
  {
    ...
    cell: ({ row }: { row: Row<InterviewData> }) => {
      ...
      return <ActionButton id={applicationId} />;
    },
    minSize: 60,
    maxSize: 60,
    size: 60,
  ...
  const filtered = useMemo(() => {
    ...
    if (q) {
      result = result.filter((d) =>
        [d.examinerName, formatDateTime(d.startTime), formatTimeRange(d.startTime, d.endTime), formatText(d.status)]
          .filter(Boolean)
