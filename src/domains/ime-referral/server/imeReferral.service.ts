...
const createIMEReferralWithClaimant = async (data: IMEFormData) => {
  try {
    ...
    // Save files to storage BEFORE transaction
    let uploadedFiles: { name: string; type: string; size: number }[] = [];
    if (data.step5?.files?.length) {
      uploadedFiles = await Promise.all(
        data.step5.files.map(async file => {
    ...
    return await prisma.$transaction(
      async tx => {
        ...
        // <-- ensured reason is set on the iMEReferral exactly as original behavior
        const referral = await tx.iMEReferral.create({
          data: {
            ...
            reason: data.step4?.reasonForReferral || '', // <-- reason set here
        ...
        let uploadedDocuments: { name: string; documentId: string }[] = [];
        if (uploadedFiles.length > 0) {
          ...
          await Promise.all(
            documents.map(doc =>
              tx.iMEReferralDocument.create({
                data: { referralId: referral.id, documentId: doc.id },
              })
            )
          );
        ...
        if (data.step3?.examTypes && data.step3.examTypes.length > 0) {
          ...
          createdExaminations = await Promise.all(
            data.step3.examTypes.map(async examType => {
              ...
              const examination = await tx.examination.create({
                data: {
                  caseNumber: `IME-${Date.now()}-${Math.random()
                    .toString(36)
                    .slice(2, 9)
                    .toUpperCase()}`,
