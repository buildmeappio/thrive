                ...
                <div className="max-sm:min-w-[600px]">
                    <div className="rounded-xl bg-[#F3F3F3]">
                        <div className="grid grid-cols-[1fr_1fr_1fr_1fr_1fr_auto] gap-4 px-6 py-1.5">
                            ...
                            <div className="text-sm font-semibold text-black max-sm:whitespace-nowrap">Company</div>
                            <div className="text-sm font-semibold text-black max-sm:whitespace-nowrap">Initials</div>
                            <div className="text-sm font-semibold text-black max-sm:whitespace-nowrap">Date</div>
                            <div className="text-sm font-semibold text-black max-sm:whitespace-nowrap">Fee</div>
                            <div className="w-6 text-sm font-semibold text-black"></div>
                        </div>
                    </div>
                    <div>
                        {NewCaseOffersData.map((offer, index) => (
                            <div
                                key={index}
                                className={`grid grid-cols-[1fr_1fr_1fr_1fr_1fr_auto] items-center gap-4 border-b border-[#B9B9B9] px-6 py-1.5 transition-colors hover:bg-gray-50
                                      ${index === NewCaseOffersData.length - 1 ? 'rounded-b-3xl border-b-0' : ''}`}
                            >
                                <div className="text-[12px] text-[#4D4D4D] max-sm:whitespace-nowrap">{offer.caseId}</div>
                                <div className="text-[12px] text-[#4D4D4D] max-sm:whitespace-nowrap">{offer.company}</div>
                                <div className="text-[12px] text-[#4D4D4D] max-sm:whitespace-nowrap">{offer.initials}</div>
                                <div className="text-[12px] text-[#4D4D4D] max-sm:whitespace-nowrap">{offer.date}</div>
                                <div className="text-[12px] text-[#008ED7] max-sm:whitespace-nowrap">{offer.fee}</div>
                                <div className="w-fit">
                                    <button
                                        onClick={handleViewAll}
